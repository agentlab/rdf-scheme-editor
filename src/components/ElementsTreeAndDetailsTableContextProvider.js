...
export function ElementsTreeAndDetailsTableContextProvider(props) {
  ...
  const onSearchInputChanged = (e) => {
    ...
    const expandedKeys = getExpandedKeys(value, rootElement[0], rootElement[0].key).filter(
  ...
  return (
    <ElementsTreeAndDetailsTableContext.Provider value={value}>
      {props.children}
    </ElementsTreeAndDetailsTableContext.Provider>
  );
}
